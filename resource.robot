...
${ONLINE_DATA_QUERY_WCS_DROPDOWN}    //*[@id="bp6-tab-panel_onlineQueryDialogTabs_0"]/div/div[1]/div/div/div[4]/div/div/button
...
Setup carta_backend And Open Browser To CARTA
    ...
    END
    ELSE
    #Open Browser    browser=${BROWSER}    service=executable_path=${CHROMEDRIVER_PATH}    options=add_argument("--use-gl=angle");add_argument("--use-angle=vulkan");add_argument("--force-color-profile=srgb");add_argument("--disable-web-security");add_argument("--force-device-scale-factor=1")
    Open Browser    browser=${BROWSER}    service=executable_path=${CHROMEDRIVER_PATH}    options=add_argument("--use-gl=angle");add_argument("--use-angle=gl");add_argument("--force-color-profile=srgb");add_argument("--disable-web-security");add_argument("--force-device-scale-factor=1")
    Set Window Size    ${WINDOW_SIZE_X}    ${WINDOW_SIZE_Y}
    END
    IF    '${BROWSER}' == 'chrome'
    Open Browser    browser=${BROWSER}    service=executable_path=${CHROMEDRIVER_PATH}    options=add_argument("--force-color-profile=srgb");add_argument("--disable-web-security")
...
Clear Input Field
    ...
    FOR    ${i}    IN RANGE    6
        Double Click Element    ${INPUT_FIELD}    
